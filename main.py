...
from scrap_cases import IndiaCovidCases
from scrap_deaths import IndiaCovidDeaths
...
def grab_training_set(datagrabber_class, grab_data_from_server=True, offline_dataset_date=""):
    ...
    return np.genfromtxt(filename, delimiter=',').astype(np.int32)
...
def print_forecast(model_name, model, beginning_day=0, limit=10):
    ...
    print("The forecast for COVID19 " + model_name + " after "+datetime.today().strftime('%Y-%m-%d')+ " for " + str(limit) + " days is:")
    print()
